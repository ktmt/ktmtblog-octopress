...
* COMPLETE: REFRESH mới hoàn toàn, Oracle sẽ query lại table theo định nghĩa của MATERIALIZED VIEW, Nếu Table chứa lượng data lớn và việc tính toán mất nhiều thời gian thì mỗi lần COMPLETE REFRESH sẽ tốn nhiều thời gian,
* FAST: REFRESH những phần mới từ lần gần đây nhất. Khi chỉ update những kết quả mới mà lần trước chưa tính toán, thời gian cho mỗi lần FAST REFRESH sẽ được rút ngắn tối thiểu.
...
Đến lần sau khi FAST REFRESH bạn sẽ có trạng thái hiện tại của data được update từ last snapshot lần trước. 
...
Còn thời điểm REFRESH (when) là khái niệm khác, định nghĩa khi nào thì MATERIALZIED VIEW được REFRESH. Có 2 mode cơ bản là manually (ON DEMAND) và automatically (ON COMMIT, DBMS_JOB).
